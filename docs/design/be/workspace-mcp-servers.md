...
管理页采用与 Skills、Files 一致的资源列表：表格行用于选择资源，行末更多菜单承载编辑、归档和删除操作。创建与编辑复用原自定义 MCP 的居中表单和输入校验，右侧资源面板只用于展示详情。选择资源时同步详情 URL，关闭面板回到列表 URL；列表的搜索与状态筛选参考 Sessions 页面，独立放在标题栏下方、表格上方，并保留游标分页。
...
- 管理页覆盖资源列表、居中创建/编辑表单、右侧详情面板、更多操作菜单、归档与游标分页。
